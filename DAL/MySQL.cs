...
namespace DAL
{
    public class MySQL<T> : IDB<T> where T : CamposControl
    {
        ...
        public T Actualizar(T entidad)
        {
            ...
            try
            {
                entidad.fecha_ejec = DateTime.Now;
            ...
            using (MySqlConnection conexion = new MySqlConnection(cadenaDeConexion))
            {
                ...
                using (MySqlCommand comando = new MySqlCommand(sql, conexion))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        foreach (var propiedad in entidad.GetType().GetProperties())
                        {
                            if (reader.HasColumn(propiedad.Name) && !reader.IsDBNull(reader.GetOrdinal(propiedad.Name)))
                            {
    ...
    // Extensión para verificar si una columna existe en el reader
